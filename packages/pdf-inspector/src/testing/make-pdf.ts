...
/**
 * WinAnsi code points that live in the 0x80-0x9F range, where WinAnsi differs
 * from Latin-1. Without these, a bullet or a euro sign in a fixture would come
 * out as '?' and a test built on it would be testing the wrong document.
 */
 ...
 * subset of WinAnsiEncoding for the accented letters), the table above covers
 * the punctuation block, and anything else becomes '?'.
 */
function winAnsiBytes(text: string): Buffer {
  const out = Buffer.alloc(text.length);
  for (let i = 0; i < text.length; i++) {
    const code = text.charCodeAt(i);
    out[i] = WIN_ANSI_SPECIALS.get(code) ?? (code < 0x100 ? code : 0x3f);
  }
  return out;
}
